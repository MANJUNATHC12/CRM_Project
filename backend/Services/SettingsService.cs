...
{
    Task<SettingsGroupDto> GetGroupAsync(string group);
    Task<Dictionary<string, string>> GetAllAsync();
    Task SaveGroupAsync(SaveSettingsDto dto);
}

public class SettingsService : ISettingsService
{
    private readonly ApplicationDbContext _ctx;
    // Default settings seeded if a key doesn't exist yet
    ...
    public async Task<SettingsGroupDto> GetGroupAsync(string group)
    {
    ...
    public async Task SaveGroupAsync(SaveSettingsDto dto)
    {
        foreach (var kv in dto.Values)
        {
            var existing = await _ctx.Settings
                .FirstOrDefaultAsync(s => s.Group == dto.Group && s.Key == kv.Key);
            if (existing != null)
