...
export default function Customers() {
  ...
  const fetchCustomers = async (currentPage = page, currentSearch = search) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_BASE_URL}/api/customers?search=${encodeURIComponent(currentSearch)}&page=${currentPage}&pageSize=10`, {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const url = modalMode === 'add' ? `${API_BASE_URL}/api/customers` : `${API_BASE_URL}/api/customers/${selectedCustomer.id}`;
  ...
  const handleAddNote = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/customers/${selectedCustomer.id}/notes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('crm_token')}`
